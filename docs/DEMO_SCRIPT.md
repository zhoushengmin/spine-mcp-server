...
**画面**：AI 对话拆图，返回部件清单（名称/尺寸/输出路径）

**对话输入**：
> 用 goblins 图集拆分部件

**旁白**：
"立绘到骨骼的关键一步——图集拆分。AI 调用 split_atlas，把一张图集拆成 41 个独立部件，支持连通域拆分，能分离贴合在一起的手臂和身体。"
...
**旁白**：
"完整的工具链、Cocos 扩展面板和中文文档都已就绪，测试全部通过。让 AI 帮你做 Spine 动画，从这里开始。"
